...
public static class DrawUtil {
    ...
    public static bool EditFloatField(string label, float fieldWidth, ref float refValue, string helpText = "",
        ...
        if (helpText != string.Empty) {
            if (hoverHelpText)
                DrawUtil.HoveredTooltip(helpText);
            else
    ...
    // porting-note(api13): upstream's ImGui.TextV has a colour overload; this build's TextV did
    // not. Same behaviour, coloured.
    public static void TextV(System.Numerics.Vector4 colour, string s) {
    ...
    // porting-note(api13): upstream delegates to ImGui.TooltipOnHover, an api15-era
    // Dalamud.Bindings.ImGui convenience wrapper. Inlined here - this is what it does.
    ...
    // porting-note(api13): DragDropSource/DragDropTarget are api15-only ImGui wrappers whose bodies
    // are not visible from metadata, so these are RECONSTRUCTED from their call sites (payload
    // carries the token index; the target reports source -> insert back to the caller) on top of the
    // drag/drop primitives this build already uses in TabFishingPresets.
    public static void DragDropSource(int index, string type, string label)
    {
        if (!ImGui.BeginDragDropSource())
            return;

        ImGuiDragDrop.SetDragDropPayload(type, index);
        ImGui.TextUnformatted(label);
        ImGui.EndDragDropSource();
    }

    public static void DragDropTarget(int index, string type, int count, Action<int, int> onMove)
    {
        if (!ImGui.BeginDragDropTarget())
            return;

        if (ImGuiDragDrop.AcceptDragDropPayload(type, out int sourceIndex))
        {
            var insertIndex = Math.Clamp(index, 0, Math.Max(0, count - 1));
            if (sourceIndex != insertIndex)
                onMove(sourceIndex, insertIndex);
        }
    ...
    public static void DrawComboSelector<T>(List<T> itemList, Func<T, string> getItemName, string selectedItem, Action<T> onSelect) {
        ...
        DrawUtil.HoveredTooltip(selectedItem);
    }
    ...
    public static void DrawComboSelectorPreset(BasePreset presetList) {
        ...
        if (!comboOpen && selectedPreset != null) {
            DrawUtil.HoveredTooltip(UIStrings.RightClickToRename);
    ...
    public static void DrawAddNewPresetButton(BasePreset presetConfig) {
        ...
        DrawUtil.HoveredTooltip(UIStrings.AddNewPreset);
    }
    ...
    public static void DrawImportExport(BasePreset basePreset) {
        try {
            using (ImRaii.Disabled(basePreset.SelectedPreset == null)) {
                ...
                DrawUtil.HoveredTooltip(UIStrings.ExportPresetToClipboard);
            ...
            DrawUtil.HoveredTooltip(UIStrings.ImportPresetFromClipboard);
