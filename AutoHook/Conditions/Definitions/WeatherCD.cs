...
public sealed class WeatherCD : SnapshottableConditionDefinition {
    ...
    private static bool WeatherMatches(List<uint> ids, uint targetWeatherId) {
        ...
        if (!Sheets.TryGetRow<Weather>(targetWeatherId, out var current))
            return false;
        var currentName = current.Name.ToString();
        return !string.IsNullOrEmpty(currentName) && ids.Any(id => Sheets.TryGetRow<Weather>(id, out var row) && row.Name.ToString() == currentName);
    }
    ...
    public override void DrawParams(Condition condition) {
        ...
        var unique = Svc.Data.GetExcelSheet<Weather>().Select(row => (Name: row.Name.ToString(), Id: (byte)row.RowId)).Where(x => !string.IsNullOrEmpty(x.Name)).DistinctBy(x => x.Name).ToDictionary(x => x.Name, x => x.Id);
        var weathers = unique.OrderBy(k => k.Key).Select(k => (Id: k.Value, Name: k.Key)).ToList();
        var label = currentId != 0 && Sheets.TryGetRow<Weather>(currentId, out var currentRow) ? currentRow.Name.ToString() : "Any weather";
